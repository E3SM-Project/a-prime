...
if ( ${generate_ohc_trends} == 1 ) then
  ...
  python python/ohc_timeseries.py --indir ${archive_dir} -c ${casename} --meshfile ${mpas_meshfile} --plots_dir ${plots_dir} --year_offset ${yr_offset} --compare_with_model "true" --casename_model_tocompare ${casename_model_tocompare} --indir_model_tocompare ${ocndir_model_tocompare} --compare_with_obs "false"
endif

if ( ${generate_sst_trends} == 1 ) then
  echo ""
  echo "Plotting SST time series..."
  python python/sst_timeseries.py --indir ${archive_dir} -c ${casename} --plots_dir ${plots_dir} --year_offset ${yr_offset} --compare_with_model "true" --casename_model_tocompare ${casename_model_tocompare} --indir_model_tocompare ${ocndir_model_tocompare}
endif
...
if ( ${generate_seaice_climo} == 1 ) then
  ...
  python python/seaice_modelvsobs.py --indir ${archive_dir} -c ${casename} --climodir ${mpas_climodir} --plots_dir ${plots_dir} --obsdir ${obs_seaicedir} --remapfile ${mpas_remapfile} --climo_year1 ${climo_yr1} --climo_year2 ${climo_yr2} --year_offset ${yr_offset}
